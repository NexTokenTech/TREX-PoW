		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				match config.role {
					...
					Role::Full =>
						service::new_full(config, false, cli.author.as_ref().map(|s| s.as_str()),cli.parallel_cpus),
					// authority node has full parts with mine
					_ => service::new_full(config, true, cli.author.as_ref().map(|s| s.as_str()),cli.parallel_cpus),
				}
